...
///
/// Proves knowledge of `(x0, x1, x2, x0_blinding, b, t1, t2)` satisfying
/// 11 algebraic relations linking the server's private key, the blinding
/// factor, and the response elements.
///
...
pub(super) fn make_response_proof(
    ...
    // 11 announcements (one per equation, in spec order)
    let announcements = [
    ...
    let c = prover.into_challenge(&announcements);
    ...
    let responses: [Scalar; 7] =
        std::array::from_fn(|i| k[i] + c * secret_witnesses[i]);
